...
async function loadSelectFromJSON(selectId, jsonPath, placeholderText = 'Loading...') {
  ...
  try {
    ...
    selectElement.innerHTML = `<option value="">Error: Could not load options. ${error.message}</option>`;
    selectElement.setAttribute('aria-label', `Error loading ${selectId}: ${error.message}`);
  }
}
